...
- [x] 읽음/안읽음 구분 + "읽음으로 표시" 버튼 (바로 아래 "읽음 표시 기능" 참고)
- [ ] 다른 인천대 공지 사이트 추가
- [ ] 필터링 정확도 다듬기
...
- 각 공지 옆에 **"읽음으로 표시"** 버튼이 있다. 누르면 그 공지가 흐리게 + 취소선으로 바뀌고 목록 맨 아래로 내려간다. 다시 누르면("읽지 않음으로 표시") 원래대로 돌아온다.
- 공지 **제목 링크를 클릭해서 실제로 열어봐도** 자동으로 읽음 처리된다.
- 읽음 상태는 **이 브라우저에만 저장**된다 (localStorage 사용, 서버·저장소에는 안 남음). 그래서:
  - 같은 브라우저로 다시 들어오면 표시가 그대로 유지된다. 매일 사이트가 새로 빌드돼도 안 사라진다.
  - 휴대폰·다른 컴퓨터·다른 브라우저에서는 따로 읽음 표시를 해야 한다 (기기 간 동기화 안 됨).
  - 브라우저 방문 기록/저장 데이터를 지우면 읽음 표시도 같이 사라진다.
- 화면 위쪽에 "안 읽은 공지 N건 / 전체 N건" 카운터가 실시간으로 갱신된다.
...
- 아무 공지나 "읽음으로 표시"를 눌러보고, 흐려지면서 아래로 내려가는지 + 위쪽 카운터 숫자가 바뀌는지 확인. 새로고침해도 그대로 유지되면 정상.
