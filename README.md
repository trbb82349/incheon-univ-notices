...
- **사이트를 처음 추가했을 때** (그 사이트 기록이 아직 하나도 없을 때): 게시판에서 가장 최근 날짜에 올라온 공지를 전부 가져온다. 그 날짜 글이 여러 페이지에 걸쳐 있으면 페이지를 계속 넘겨서라도 다 가져온다.
- **그 다음부터**: 이전에 이미 모아둔 공지는 절대 지우지 않고 그대로 두고, 그 이후 새로 올라온 공지만 찾아서 추가한다. 이미 알고 있는 글(링크로 구분)을 만나면 그 뒤로는 다 예전 글이라는 뜻이므로 페이지 넘기기를 멈춘다.
